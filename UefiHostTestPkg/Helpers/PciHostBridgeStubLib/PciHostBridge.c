  ...
  )
{
  ...
  for (Index = 0; Index < RootBridgeCount; Index++) {
    ...
    //
    RootBridge = HostBridgeCreateRootBridge (&RootBridges[Index]);
  ...
  //
  if (!ResourceAssigned) {
    HostBridge->ResAlloc.NotifyPhase = HostBridgeNotifyPhase;
    ...
    HostBridge->ResAlloc.PreprocessController = HostBridgePreprocessController;
