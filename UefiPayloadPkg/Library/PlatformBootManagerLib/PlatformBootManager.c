...
)
{
  ...
  // Register UEFI Shell
  //
  PlatformRegisterFvBootOption (PcdGetPtr (PcdShellFile), L"UEFI Shell", LOAD_OPTION_ACTIVE, (UINTN) -1);
  
  //
  // Register costum readyToBoot function as the first boot option(hidden boot option) to run before normal EFI Boot
  //
  PlatformRegisterFvBootOption (PcdGetPtr (PcdCustomBootFile), L"CUSTOM BOOT", LOAD_OPTION_ACTIVE, (UINTN) 0);
