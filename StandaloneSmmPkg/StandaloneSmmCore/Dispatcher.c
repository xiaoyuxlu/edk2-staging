  ...
  )
{
  ...
  DEBUG ((EFI_D_INFO, "SmmLoadImage - %g\n", &DriverEntry->FileName));
  ...
  )
{
  ...
  DEBUG ((EFI_D_INFO, "SmmDispatcher\n"));

  if (!gRequestDispatch) {
    DEBUG ((EFI_D_INFO, "  !gRequestDispatch\n"));
    return EFI_NOT_FOUND;
  }

  if (gDispatcherRunning) {
    DEBUG ((EFI_D_INFO, "  gDispatcherRunning\n"));
    //
  ...
  do {
    ...
    //
    DEBUG ((EFI_D_INFO, "  Drain the Scheduled Queue\n"));
    while (!IsListEmpty (&mScheduledQueue)) {
      ...
      DEBUG ((EFI_D_INFO, "  DriverEntry (Scheduled) - %g\n", &DriverEntry->FileName));
      ...
      //
      if (mEfiSystemTable == NULL) {
        DEBUG ((EFI_D_INFO, "StartImage - 0x%x (Standalone Mode)\n", DriverEntry->ImageEntryPoint));
        Status = ((SMM_IMAGE_ENTRY_POINT)(UINTN)DriverEntry->ImageEntryPoint)(DriverEntry->ImageHandle, &gSmmCoreSmst);
      } else {
        DEBUG ((EFI_D_INFO, "StartImage - 0x%x (Tradition Mode)\n", DriverEntry->ImageEntryPoint));
    ...
    //
    DEBUG ((EFI_D_INFO, "  Search DriverList for items to place on Scheduled Queue\n"));
    ReadyToRun = FALSE;
    for (Link = mDiscoveredList.ForwardLink; Link != &mDiscoveredList; Link = Link->ForwardLink) {
      DriverEntry = CR (Link, EFI_SMM_DRIVER_ENTRY, Link, EFI_SMM_DRIVER_ENTRY_SIGNATURE);
      DEBUG ((EFI_D_INFO, "  DriverEntry (Discovered) - %g\n", &DriverEntry->FileName));
  ...
  //
  DEBUG ((EFI_D_INFO, "  no more SMM driver to dispatch, stop the dispatch request\n"));
  ...
  )
{
  ...
  DEBUG ((EFI_D_INFO, "FvIsBeingProcesssed - 0x%08x\n", FvHandle));
  ...
  )
{
  ...
  DEBUG ((EFI_D_INFO, "SmmAddToDriverList - %g (0x%08x)\n", DriverName, Pe32Data));
  ...
  )
{
  ...
  DEBUG ((EFI_D_INFO, "SmmDriverDispatchHandler\n"));
...
{
  EFI_STATUS                            Status;
  EFI_SMM_COMMUNICATE_FV_DISPATCH_DATA  *CommunicationFvDispatchData;
  EFI_FIRMWARE_VOLUME_HEADER            *FwVolHeader;

  DEBUG ((EFI_D_INFO, "SmmFvDispatchHandler\n"));
  ...
  SmmCoreFfsFindSmmDriver (FwVolHeader);
