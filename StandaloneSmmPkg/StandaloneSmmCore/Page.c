  ...
  )
{
  ...
  *MapKey = mMapKey;
  ...
  if (*MemoryMapSize < Size * mSmramRangeCount) {
    *MemoryMapSize = Size * mSmramRangeCount;
    return EFI_BUFFER_TOO_SMALL;
  }

  //
  // TBD - Fill Allocate/Free memory map
  //
  for (Index = 0; Index < mSmramRangeCount; Index++) {
    if ((mSmramRanges[Index].RegionState & (EFI_ALLOCATED | EFI_NEEDS_TESTING | EFI_NEEDS_ECC_INITIALIZATION)) != 0) {
      MemoryMap->Type = EfiRuntimeServicesData;
    } else {
      MemoryMap->Type = EfiConventionalMemory;
    }
    MemoryMap->PhysicalStart = mSmramRanges[Index].CpuStart;
    MemoryMap->VirtualStart  = 0;
    MemoryMap->NumberOfPages = EFI_SIZE_TO_PAGES(mSmramRanges[Index].PhysicalSize);
