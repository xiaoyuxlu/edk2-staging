/** @file
  HOB Library implemenation for Standalone Phase.
  ...
  )
{
  ...
  for (Index = 0; Index < gSmst->NumberOfTableEntries; Index++) {
    if (CompareGuid (&gEfiHobListGuid, &gSmst->SmmConfigurationTable[Index].VendorGuid)) {
      mHobList = gSmst->SmmConfigurationTable[Index].VendorTable;
      break;
    }
  }
