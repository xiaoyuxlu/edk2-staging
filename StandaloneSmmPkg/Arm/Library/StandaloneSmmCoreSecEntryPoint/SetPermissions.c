  ...
  )
{
  ...
  Status = FfsFindNextFile(
	    ...
	    &FileHeader);
  if (EFI_ERROR(Status)) {
    DEBUG((EFI_D_ERROR, "Unable to locate Standalone MM FFS file - 0x%x\n",
      Status));
    return Status;
  }
  ...
  if (EFI_ERROR(Status)) {
      DEBUG((EFI_D_ERROR, "Unable to locate Standalone MM Section data - 0x%x\n",
	   Status));
    return Status;
  }

  DEBUG((EFI_D_INFO, "Found Standalone MM PE data - 0x%x\n", *TeData));
  return Status;
}
...
{
  EFI_STATUS                   Status;
  PE_COFF_LOADER_IMAGE_CONTEXT TmpContext = {0};
  ...
  if (EFI_ERROR(Status)) {
    DEBUG((EFI_D_ERROR, "Unable to locate Standalone MM Core PE-COFF Image information - 0x%x\n", Status));
    return Status;
  }

  Status = GetPeCoffSectionInformation(ImageContext, &TmpContext, SectionHeaderOffset, NumberOfSections);
  if (EFI_ERROR(Status)) {
    DEBUG((EFI_D_ERROR, "Unable to locate Standalone MM Core PE-COFF Section information - 0x%x\n", Status));
    return Status;
  }

  DEBUG((EFI_D_INFO, "Standalone MM Core PE-COFF SectionHeaderOffset - 0x%x, NumberOfSections - %d\n", *SectionHeaderOffset, *NumberOfSections));
