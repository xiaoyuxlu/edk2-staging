  ...
  )
{
  ...
  *Smst = (EFI_SMM_SYSTEM_TABLE2 *)(UINTN)gSmmCorePrivate->Smst;
...
{
  EFI_SMM_COMMUNICATE_FV_DISPATCH  CommunicationFvDispatch;
  UINTN                            Size;
  ...
  for (Hob.Raw = GetHobList(); !END_OF_HOB_LIST(Hob); Hob.Raw = GET_NEXT_HOB(Hob)) {
    if (GET_HOB_TYPE (Hob) == EFI_HOB_TYPE_FV2) {
      CopyGuid (&CommunicationFvDispatch.HeaderGuid, &gSmmFvDispatchGuid);
      CommunicationFvDispatch.MessageLength = sizeof(EFI_SMM_COMMUNICATE_FV_DISPATCH_DATA);
  ...
  )
{
  ...
  GuidHob = GetFirstGuidHob (&gSmmCoreDataHobGuid);
  if (GuidHob == NULL) {
    return EFI_UNSUPPORTED;
  }
