...
--*/
{
  ...
  // Create node
  //
  NewRivlVariable = AllocatePool (sizeof (RIVL_VARIABLE));
  ...
  case RIVL_VAR_ATTRIB_VARIABLE:
    NewRivlVariable->Address = AllocatePool (TotalSize);
  ...
  case RIVL_VAR_ATTRIB_POINTER:
    NewRivlVariable->Address = AllocatePool (sizeof (VOID *));
    if (NewRivlVariable->Address == NULL) {
      FreePool (NewRivlVariable);
      return EFI_OUT_OF_RESOURCES;
    }
  ...
  if (OldRivlVariable != NULL) {
    ...
    FreePool (NewRivlVariable->Address);
    FreePool (NewRivlVariable);
...
--*/
{
  ...
  case RIVL_VAR_ATTRIB_POINTER:
    if (OldRivlVariable->Address != NULL) {
      FreePool (OldRivlVariable->Address);
    }
    break;
