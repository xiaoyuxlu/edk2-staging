  ...
  )
{
  ...
  for (Index = 0; Index < gST->NumberOfTableEntries; Index++) {
    ...
    // Get library instance No.
      //
      Link = (LIST_ENTRY *) gST->ConfigurationTable[Index].VendorTable;
      ...
      while (ExecuteInfo->Index < NoInstance) {
        ...
        //
        InstanceIndex = 0;
        Link = ((LIST_ENTRY *) gST->ConfigurationTable[Index].VendorTable)->ForwardLink;
...
--*/
{
  ...
  //
  if (CompareGuid (Guid, &gEfiGenericCategoryGuid)) {
    while (ExecuteInfo->Iteration < ExecuteInfo->TestCase->Iterations) {
      //
  ...
  } else if (CompareGuid (Guid, &gEfiBootServicesCategoryGuid)) {
    while (ExecuteInfo->Iteration < ExecuteInfo->TestCase->Iterations) {
      //
      // Boot services test
      //
      ...
      //
      Status = ExecuteBbTestInstance (ExecuteInfo, gBS, NULL);
      if (EFI_ERROR (Status)) {
        EFI_SCT_DEBUG ((EFI_SCT_D_ERROR, L"Execute a BB test instance - %r", Status));
        return Status;
      }
  ...
  } else if (CompareGuid (Guid, &gEfiRuntimeServicesCategoryGuid)) {
    while (ExecuteInfo->Iteration < ExecuteInfo->TestCase->Iterations) {
      //
      ...
      //
      Status = ExecuteBbTestInstance (ExecuteInfo, gRT, NULL);
      if (EFI_ERROR (Status)) {
        EFI_SCT_DEBUG ((EFI_SCT_D_ERROR, L"Execute a BB test instance - %r", Status));
        return Status;
      }
...
--*/
{
  ...
  if (Handle != NULL) {
    ...
    if (DevicePath != NULL) {
      ConfigData->DevicePath = ConvertDevicePathToText (DevicePath, FALSE, FALSE);
    } else {
...
--*/
{
  ...
  //
  if (ConfigData->SystemLogFile.FileName != NULL) {
    FreePool (ConfigData->SystemLogFile.FileName);
    ConfigData->SystemLogFile.FileName = NULL;
  }

  if (ConfigData->SystemKeyFile.FileName != NULL) {
    FreePool (ConfigData->SystemKeyFile.FileName);
    ConfigData->SystemKeyFile.FileName = NULL;
  }

  if (ConfigData->CaseLogFile.FileName != NULL) {
    FreePool (ConfigData->CaseLogFile.FileName);
    ConfigData->CaseLogFile.FileName = NULL;
  }

  if (ConfigData->CaseKeyFile.FileName != NULL) {
    FreePool (ConfigData->CaseKeyFile.FileName);
    ConfigData->CaseKeyFile.FileName = NULL;
  }

  if (ConfigData->TestCategory != NULL) {
    FreePool (ConfigData->TestCategory);
    ConfigData->TestCategory = NULL;
  }

  if (ConfigData->DevicePath != NULL) {
    FreePool (ConfigData->DevicePath);
    ConfigData->DevicePath = NULL;
  }
