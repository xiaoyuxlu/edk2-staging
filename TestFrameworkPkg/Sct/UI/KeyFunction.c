...
--*/
{
  ...
  gST->ConOut->SetCursorPosition (gST->ConOut, XLeft, Ypos);
  ...
  while (*TempStr != L'\0' && CurrentYpos <= Y1) {
    ...
    if (StrLen (TempStr) + CurrentXpos > X1 + 1 ) {
      ...
      if (StrLen (TempStr) <= X1 - X0 + 1) {
        ...
        gBS->SetMem (OneLine, (X1 - X0 + 2) * sizeof(CHAR16), 0);

        if (CurrentXpos < X1) {
          gBS->CopyMem (OneLine, TempStr, (X1 - CurrentXpos) * sizeof(CHAR16));
        ...
        while ((StrLen (TempStr) > X1 - X0 + 1)&& CurrentYpos <= Y1 ) {
          ...
          gBS->CopyMem (OneLine, TempStr, (X1 - X0) * sizeof(CHAR16));
          OneLine[X1 - X0]     = L'-';
          OneLine[X1 - X0 + 1] = L'\0';
...
--*/
{
  ...
  if (MenuItem == Page->Body.CurrentSelected) {
    ...
    if (StrLen (Buffer) == 0 ) {
      ...
      Status = gST->ConOut->SetAttribute (gST->ConOut, Attrib);
      return Status;
    }
  } else {
    if (StrLen (Buffer) == 0) {
