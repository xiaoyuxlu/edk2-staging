...
--*/
{
  ...
  // Header
  //
  FreePool (Page->Header.HeaderString.Text);

  //
  // Body
  //
  FreePool (Page->Body.MenuItemHeader.Text);
  FreePool (Page->Body.DescriptionHeader.Text);
  ...
  while (HotKey != NULL) {
    ...
    FreePool (HotKey->HotKeyName.Text);
    FreePool (HotKey->HotKeyDesc.Text);
    FreePool (HotKey);
...
--*/
{
  ...
  gBS->SetMem(MenuItem, sizeof (EFI_MENU_ITEM), 0);
...
--*/
{
  ...
  gBS->SetMem (TempValueQueue, sizeof (EFI_ITEM_VALUE_QUEUE), 0);
...
*/
{
  ...
  while (NextValueQueue != NULL) {
    ...
    if (NextValueQueue->StringValue != NULL) {
      FreePool(NextValueQueue->StringValue);
    }

    FreePool (NextValueQueue);
