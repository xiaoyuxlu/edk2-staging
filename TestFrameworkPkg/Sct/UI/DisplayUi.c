...
--*/
{
  ...
  //
  gST->ConOut->SetAttribute (gST->ConOut,EFI_MENUPAGE_HEADER_BGCOLOR);
...
--*/
{
  ...
  gST->ConOut->SetAttribute (gST->ConOut, EFI_MENUPAGE_FOOTER_BGCOLOR);
...
--*/
{
  ...
  switch (MENU_ITEM_TYPE (MenuItem->ItemType)) {
    ...
    case EFI_ITEM_HAVE_COMBOLIST:
      ...
      Buffer[Index] = L'\0';
      Attrib = gST->ConOut->Mode->Attribute;
      gST->ConOut->SetAttribute (
            gST->ConOut,
...
--*/
{
    ...
    //
    gMode      = gST->ConOut->Mode->Mode;
    gConAttrib = gST->ConOut->Mode->Attribute;
    gMenuPage  = NULL;
    gMenuExit  = FALSE;
    //
    //disable cursor
...
--*/
{
  ...
  //
  if (gST->ConOut->Mode->Attribute != gConAttrib) {
    gST->ConOut->SetAttribute (gST->ConOut, gConAttrib);
  }
  ...
  //
  if (gMode != gST->ConOut->Mode->Mode) {
    gST->ConOut->SetMode (gST->ConOut, gMode);
  }
  gST->ConOut->ClearScreen (gST->ConOut);
  return EFI_SUCCESS;
}
