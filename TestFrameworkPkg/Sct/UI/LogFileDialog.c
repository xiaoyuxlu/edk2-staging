  ...
  )
{
  ...
  if (ItemContext->DevicePathStr != NULL) {
    FreePool (ItemContext->DevicePathStr);
  ...
  while (DialogItem != NULL) {
    ...
    if (ItemContext != NULL) {
      ...
      if (ItemContext->DevicePath != NULL) {
        FreePool (ItemContext->DevicePath);
...
--*/
{
  ...
  if (Length > EFI_MAX_ITEM_VALUE_LENGTH) {
    ...
    gBS->CopyMem (Buffer, LogFileDialog->FileName, (Length + 1) * sizeof(CHAR16));
  }
  //
...
--*/
{
  ...
  WAITING    = TRUE;
  TempStr[0] = L'\0';
  gBS->SetMem (&Key, sizeof(EFI_INPUT_KEY), 0);
  ...
  //enable cursor.
  //
  gST->ConOut->SetCursorPosition (gST->ConOut, XLeft + XOffSetInScreen, Ypos);
  gST->ConOut->EnableCursor (gST->ConOut,TRUE);

  while (WAITING) {
    Status = gBS->WaitForEvent (1, &(gST->ConIn->WaitForKey), &Num);
    if (!EFI_ERROR(Status)) {
    ...
    switch (Key.ScanCode) {
      ...
      case SCAN_F1:
        if (Key.ScanCode == SCAN_ESC ||
          ...
          gBS->CopyMem (TempStr, Buffer, EFI_MAX_ITEM_VALUE_LENGTH * sizeof(CHAR16));
...
--*/
{
  ...
  gBS->SetMem (DialogItem, sizeof(EFI_FILE_DIALOG_ITEM), 0);
...
--*/
{
  ...
  TempStr = AllocateZeroPool (Size1 + Size2 + sizeof (CHAR16));
  if (TempStr == NULL) {
    FreePool (Str);
    return NULL;
  }
  ...
  while (*Ptr != 0) {
    if (*Ptr == '\\' && *(Ptr+1) == '.' && *(Ptr+2) == '.' && *(Ptr+3) != 0) {
      ...
      //
      StrCpy (TempStr, Ptr + 3);
      StrCpy (LastSlash, TempStr);
      Ptr = LastSlash;
...
--*/
{
  ...
  DirInfo = FileHandleGetInfo (NewDir);
  if (!DirInfo) {
    return FALSE;
  }
  ...
  for (Pass = 1; Pass <= PassNumber; Pass++) {
    ...
    for (;;) {
      ...
      gBS->SetMem(Context,sizeof(EFI_FILE_DIALOG_ITEM_CONTEXT),0);
      ...
      Context->DevicePathStr = ConvertDevicePathToText (Context->DevicePath, FALSE, FALSE);
...
--*/
{
  ...
  gBS->SetMem ((VOID *)&ContextCopy, sizeof(EFI_FILE_DIALOG_ITEM_CONTEXT), 0);
    ...
    if (Status == EFI_NOT_FOUND) {
      if (DirDialog->Type == EFI_FILE_DIALOG_TYPE_SAVE_FILE) {
        ...
        if (DialogContext->FileName != NULL) {
          FreePool (DialogContext->FileName);
        }
        if (DialogContext->DevicePath != NULL) {
          FreePool (DialogContext->DevicePath);
        }
      ...
      //
      FileInfo = FileHandleGetInfo (FHandle);
      if (FileInfo == NULL) {
        break;
      }
      ...
      if ((FileInfo->Attribute & EFI_FILE_DIRECTORY )&&
        ...
        if (SctStrBeginWith (DirDialog->FileName, L"\\")) {
          ...
          if ( ContextCopy.FileName == NULL) {
            FreePool (ContextCopy.DevicePath);
            FHandle->Close(FHandle);
            break;
          }
        }
...
--*/
{
  ...
  ConAttrib             = gST->ConOut->Mode->Attribute;
  Status = gBS->AllocatePool (
                 EfiBootServicesData,
  ...
  gBS->SetMem (FileSystemDialog, sizeof(EFI_FILE_DIALOG), 0);
  ...
  //
  gBS->CopyMem (DirDialog, FileSystemDialog, sizeof(EFI_FILE_DIALOG));
  ...
  //
  gST->ConOut->SetAttribute ( gST->ConOut,ConAttrib);
