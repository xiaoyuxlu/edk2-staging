...
AddRootTestNode (
  ...
  IN EFI_GUID                     *Guid,
  IN OUT LIST_ENTRY           *TestNodeList,
  OUT EFI_SCT_TEST_NODE           **RootNode
  );
...
--*/
{
  ...
  case EFI_SCT_TEST_FILE_TYPE_IHV_BLACK_BOX:
    ...
    if (EFI_ERROR (Status)) {
      ...
      FreePool (CategoryName);
      FreePool (CategoryDescription);
      continue;
    }
...
--*/
{
  ...
  if (TestNode->Name != NULL) {
    FreePool (TestNode->Name);
    TestNode->Name = NULL;
  }

  if (TestNode->Description != NULL) {
    FreePool (TestNode->Description);
