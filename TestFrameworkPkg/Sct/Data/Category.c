...
InsertSingleCategoryData (
  IN OUT LIST_ENTRY           *CategoryList,
...
--*/
{
  ...
  for (Link = CategoryList->ForwardLink; Link != CategoryList; Link = Link->ForwardLink) {
    ...
    if (CompareGuid (Guid, &TempCategory->CategoryGuid)) {
...
--*/
{
  ...
  for (Link = CategoryList->ForwardLink; Link != CategoryList; Link = Link->ForwardLink) {
    ...
    if (CompareGuid (&OldCategory->CategoryGuid, &Category->CategoryGuid)) {
...
--*/
{
  ...
  if (Category->Name != NULL) {
    FreePool (Category->Name);
    Category->Name = NULL;
  }

  if (Category->Description != NULL) {
    FreePool (Category->Description);
