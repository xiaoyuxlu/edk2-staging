...
//
#define TEST_LOGGING_PRIVATE_DATA_SIGNATURE   SIGNATURE_32('T','L','L','P')
...
--*/
{
  ...
  if (Private->VerboseLevel >= EFI_VERBOSE_LEVEL_EXHAUSTIVE) {
    UnicodeSPrint (Buffer, EFI_MAX_PRINT_BUFFER, L"Enter function %s: ", FunctionName);
    Status = TllWriteLogFile (Private, Buffer);
    if (EFI_ERROR (Status)) {
      return Status;
    }
    ...
    VA_START(Marker, Fmt);
    UnicodeVSPrint (Buffer, EFI_MAX_PRINT_BUFFER, Fmt, Marker);
    VA_END (Marker);
...
--*/
{
  ...
  if (Private->VerboseLevel >= EFI_VERBOSE_LEVEL_EXHAUSTIVE) {
    UnicodeSPrint (Buffer, EFI_MAX_PRINT_BUFFER, L"Exit function %s: ", FunctionName);
    Status = TllWriteLogFile (Private, Buffer);
    if (EFI_ERROR (Status)) {
      return Status;
    }
...
--*/
{
  ...
  if (VerboseLevel <= Private->VerboseLevel) {
    ...
    for (BitPos = 0; BitPos < Length; BitPos++) {
      if (BitMask & (1<<BitPos)) {
        if (Buffer[0] == L'\0') {
          UnicodeSPrint (Buffer, EFI_MAX_PRINT_BUFFER, L"%02dH", BitPos);
        } else {
          UnicodeSPrint (Buffer, EFI_MAX_PRINT_BUFFER, L"%s %02dH", Buffer, BitPos);
        }
      }
    }
    if (Buffer[0] == L'\0') {
      StrCpy (Buffer, L"none\r\n");
    } else {
      StrCat (Buffer, L"\r\n");
    }
    Status = TllWriteLogFile (Private, Buffer);
...
--*/
{
  ...
  if (VerboseLevel <= Private->VerboseLevel) {
    for (Index = 0; Index < Length; Index++) {
      ...
      //
      if (HexBuffer[0] == L'\0') {
        UnicodeSPrint (HexBuffer, EFI_MAX_PRINT_BUFFER/4, L"%04xH",
                       Buffer[Index]);
      } else {
        UnicodeSPrint (HexBuffer, EFI_MAX_PRINT_BUFFER/4, L"%s %04xH",
                       HexBuffer, Buffer[Index]);
      }
      if ((Buffer[Index] >= 0x0020) && (Buffer[Index] < 0x007F)) {
        UnicodeSPrint (AsciiBuffer, EFI_MAX_PRINT_BUFFER/4, L"%s%c",
                       AsciiBuffer, Buffer[Index]);
      } else {
        UnicodeSPrint (AsciiBuffer, EFI_MAX_PRINT_BUFFER/4, L"%s.",
                       AsciiBuffer);
      }
      //
      ...
      if (Index % 8 == 7) {
        if ((Flags & EFI_DUMP_HEX) && (Flags & EFI_DUMP_ASCII)) {
          UnicodeSPrint (OutBuffer, EFI_MAX_PRINT_BUFFER, L"%08x: %s - %s\n", Index/8*8,
                         HexBuffer, AsciiBuffer);
        } else if (Flags & EFI_DUMP_HEX) {
          UnicodeSPrint (OutBuffer, EFI_MAX_PRINT_BUFFER, L"%08x: %s\n", Index/8*8,
                         HexBuffer);
        } else {
          UnicodeSPrint (OutBuffer, EFI_MAX_PRINT_BUFFER, L"%08x: %s\n", Index/8*8,
                         AsciiBuffer);
...
--*/
{
  ...
  CopyMem (&Private->EntryId, Config->EntryId, sizeof(EFI_GUID));
...
--*/
{
  ...
  ZeroMem (Private, sizeof(TEST_LOGGING_PRIVATE_DATA));
