...
#define EFI_MAX_PRINT_BUFFER                512

//
// Private data structures
//

#define STANDARD_TEST_PRIVATE_DATA_SIGNATURE  SIGNATURE_32('S','T','S','L')
  ...
  )
{
  ...
  while (!CompareGuid (Guid, &gZeroGuid)) {
    NoGuids ++;
    Guid ++;
  }
  ...
  CopyMem (Buffer, Guids, (NoGuids + 1) * sizeof(EFI_GUID));
  ...
  )
{
  ...
  // System log file
  //
  UnicodeSPrint (String, EFI_MAX_PRINT_BUFFER, L"Logfile: \"%s\"\n",
...
--*/
{
  ...
  // Check the parameter
  //
  if (StrLen (Description) + 14 > EFI_MAX_PRINT_BUFFER) {
  ...
  case EFI_TEST_ASSERTION_PASSED:
    StrCpy (AssertionType, L"PASS");
    Private->PassCount ++;
    break;
  case EFI_TEST_ASSERTION_WARNING:
    StrCpy (AssertionType, L"WARNING");
    Private->WarningCount ++;
    break;
  case EFI_TEST_ASSERTION_FAILED:
    StrCpy (AssertionType, L"FAILURE");
    Private->FailCount ++;
    break;
  default:
  ...
  UnicodeSPrint (Buffer, EFI_MAX_PRINT_BUFFER, L"%s -- %s\n", Description, AssertionType);
  Status = StslWriteLogFile (Private, Buffer);
  if (EFI_ERROR (Status)) {
    return Status;
  }

  UnicodeSPrint (Buffer, EFI_MAX_PRINT_BUFFER, L"%g\n", &EventId);
  Status = StslWriteLogFile (Private, Buffer);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  UnicodeVSPrint (Buffer, EFI_MAX_PRINT_BUFFER, Detail, Marker);
  VA_END (Marker);

  if ( StrLen (Buffer) + 5 < EFI_MAX_PRINT_BUFFER ) {
    StrCat (Buffer, L"\r\n\r\n");
  }
  ...
  //
  UnicodeSPrint (Buffer, EFI_MAX_PRINT_BUFFER, L"%g:%s|%s:",
  ...
  if ( StrLen (Buffer) + 3 < EFI_MAX_PRINT_BUFFER ) {
    StrCat (Buffer, L"\r\n");
  }
...
--*/
{
  ...
  if (VerboseLevel <= Private->VerboseLevel) {
    ...
    UnicodeVSPrint (Buffer, EFI_MAX_PRINT_BUFFER, Message, Marker);
    VA_END (Marker);
...
--*/
{
  ...
  if ((FileConf->DevicePath != NULL) && (FileConf->FileName != NULL)) {
    PrivateFileConf->DevicePath = DuplicateDevicePath (FileConf->DevicePath);
  ...
  CopyGuid (&Private->EntryId, Config->EntryId);
...
--*/
{
  ...
  if (Private->IsRecovery) {
    ...
    UnicodeSPrint (Buffer, EFI_MAX_PRINT_BUFFER, L"      *********RECOVERY*********\n");
    ...
    UnicodeSPrint (Buffer, EFI_MAX_PRINT_BUFFER, L"%s\n", Private->EntryName);
    StslWriteLogFile (Private, Buffer);
    UnicodeSPrint (Buffer, EFI_MAX_PRINT_BUFFER, L"Revision 0x%08x\n", Private->TestRevision);
    StslWriteLogFile (Private, Buffer);
    UnicodeSPrint (Buffer, EFI_MAX_PRINT_BUFFER, L"Test Entry Point GUID: %g\n", &Private->EntryId);
    StslWriteLogFile (Private, Buffer);
    UnicodeSPrint (Buffer, EFI_MAX_PRINT_BUFFER, L"Test Support Library GUIDs: \n");
    StslWriteLogFile (Private, Buffer);
    ...
    UnicodeSPrint (Buffer, EFI_MAX_PRINT_BUFFER, L"%s\n", Private->BiosId);
    StslWriteLogFile (Private, Buffer);
    UnicodeSPrint (Buffer, EFI_MAX_PRINT_BUFFER, L"Test Configuration #%d\n", Private->ConfigurationNumber);
    StslWriteLogFile (Private, Buffer);
    ...
    CurrentTime = &Private->StartTime;
    gRT->GetTime (CurrentTime, NULL);
    UnicodeSPrint (Buffer, EFI_MAX_PRINT_BUFFER, L"Test Started: %t\n", CurrentTime);
    StslWriteLogFile (Private, Buffer);
...
--*/
{
  ...
  //
  UnicodeSPrint (Buffer, EFI_MAX_PRINT_BUFFER,
  ...
  StslWriteLogFile (Private, Buffer);
  UnicodeSPrint (Buffer, EFI_MAX_PRINT_BUFFER, L"  Passes........... %d\n", Private->PassCount);
  StslWriteLogFile (Private, Buffer);
  UnicodeSPrint (Buffer, EFI_MAX_PRINT_BUFFER, L"  Warnings......... %d\n", Private->WarningCount);
  StslWriteLogFile (Private, Buffer);
  UnicodeSPrint (Buffer, EFI_MAX_PRINT_BUFFER, L"  Errors........... %d\n", Private->FailCount);
  StslWriteLogFile (Private, Buffer);
  ...
  StslWriteLogFileName (Private);
  gRT->GetTime (&CurrentTime, NULL);
  UnicodeSPrint (Buffer, EFI_MAX_PRINT_BUFFER, L"Test Finished: %t\n", &CurrentTime);
  StslWriteLogFile (Private, Buffer);
...
--*/
{
  ...
  ZeroMem (Private, sizeof(STANDARD_TEST_PRIVATE_DATA));
