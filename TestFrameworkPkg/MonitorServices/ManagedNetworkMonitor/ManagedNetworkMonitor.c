...
STATIC EFI_MANAGED_NETWORK_PROTOCOL         *mMnp;
STATIC UINTN                                mContext;
...
Error:
  if (gManagedNetworkMonitorInterface != NULL) {
    FreePool (gManagedNetworkMonitorInterface);
  }
...
--*/
{
  ...
  if(!EFI_ERROR(Status)) {
    CopyMem (&mDestinationAddress, &Data, sizeof (EFI_MAC_ADDRESS));
  }
  ...
  )
{
  ...
  if (EFI_ERROR(Status)) {
    ...
    ZeroMem (&mDestinationAddress, sizeof(EFI_MAC_ADDRESS));
  }
...
--*/
{
  ...
  MnpBufferOutSize = StrLen(Buffer);
  MnpBufferOut = AllocatePool (MnpBufferOutSize + 1);
...
//      SET_FLAG_MF (FragFlag.LLFlag);
    ...
    // Build data
    //
     FragmentBuffer = AllocatePool (PacketLength + sizeof (EAS_MNP_FRAG_FLAG) + sizeof (EAS_APP_FLAG));
  if(NULL == FragmentBuffer) {
      EFI_ENTS_DEBUG ((EFI_ENTS_D_ERROR, L"AllocatePool Error"));
      return EFI_OUT_OF_RESOURCES;
    }

    CopyMem (&TxData, &mMnpTxDataTemplate, sizeof (EFI_MANAGED_NETWORK_TRANSMIT_DATA));
    ...
    //
    mContext = 0;
    Status  = mMnp->Transmit (mMnp, &TxToken);
    if (EFI_ERROR (Status)) {
      RecycleTxBuffer(FragmentBuffer);
      EFI_ENTS_DEBUG ((EFI_ENTS_D_ERROR, L"mMnp->Transmit Error"));
      return Status;
    }
    //
...
--*/
{
  ...
  CopyMem (&mSourceAddress, &SnpModeData.CurrentAddress, sizeof (EFI_MAC_ADDRESS));
...
--*/
{
  ...
  // Build Fragment Flag
  //
  CopyMem (&FragFlag.LLFlag, RxData->PacketData, sizeof (EAS_MNP_FRAG_FLAG));
...
//  IsOver            = HAS_FLAG_MF (FragFlag.LLFlag) ? FALSE : TRUE;
  ...
  // Build App Flag
  //
  CopyMem (&AppFlag.LLFlag, (UINT8 *) RxData->PacketData + sizeof (EAS_MNP_FRAG_FLAG), sizeof (EAS_APP_FLAG));
  ...
  if (PacketStartPoint == 0) {
    ...
    (mDestinationAddress.Addr[5] != ((CHAR8 *)(RxData->SourceAddress))[5])
      ...
      CopyMem(&mDestinationAddress, RxData->SourceAddress, sizeof(EFI_MAC_ADDRESS));
      Status = SetContextRecord(
                 gntDevicePath,
                 SCT_PASSIVE_MODE_RECORD_FILE,
                 ENTS_SERVER_MAC_ADDRESS_NAME,
...
RESTART_RECEIVE:
  RecycleRxBuffer ();
  Status = mMnp->Receive (mMnp, &RxToken);
  return ;
}
...
--*/
{
  ...
  Buffer = AllocatePool (sizeof (EAS_MNP_FRAG_FLAG));
  if(Buffer == NULL) {
    EFI_ENTS_DEBUG ((EFI_ENTS_D_ERROR, L"AllocatePool Error"));
    return EFI_OUT_OF_RESOURCES;
  }
  ...
  // Build data
  //
  CopyMem (&TxLLData, &mMnpTxDataTemplate, sizeof (EFI_MANAGED_NETWORK_TRANSMIT_DATA));
  ...
  CopyMem (TxLLData.FragmentTable[0].FragmentBuffer, &FragFlag.LLFlag, sizeof (EAS_MNP_FRAG_FLAG));
  ...
  Status                  = mMnp->Transmit (mMnp, &TxLLToken);
  if (EFI_ERROR (Status)) {
