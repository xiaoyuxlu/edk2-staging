...
--*/
{
  ...
  if (CompareMem (Buffer + Size - SIGNAL_TAG_LENGTH, gSerialSignalStart, SIGNAL_TAG_LENGTH) == 0) {
    EntsPrint ((L"\nget sig begin\n"));
    return TRUE;
  }
...
--*/
{
  ...
  if (CompareMem (Buffer + Size - SIGNAL_TAG_LENGTH, gSerialSignalEnd, SIGNAL_TAG_LENGTH) == 0) {
    EntsPrint ((L"\nget sig end\n"));
    return TRUE;
  }
...
--*/
{
  ...
  CopyMem (Buffer + (*BufferSize), &BufferTmp, Size);
  *BufferSize += Size;
...
--*/
{
  ...
  *Size   = 0;
  ...
  do {
    ...
    if (IsComdEnd (SerialBufferOut, *Size)) {
      SetMem (SerialBufferOut + (*Size) - SIGNAL_TAG_LENGTH, SIGNAL_TAG_LENGTH, 0);
  ...
  *Buffer = AllocatePool((*Size + 1) * sizeof(CHAR16));
  if (*Buffer == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }
  Status = AsciiStrToUnicodeStrS (SerialBufferOut, *Buffer, *Size + 1);
  if (EFI_ERROR (Status)) {
    return Status;
  }

  SetMem (SerialBufferOut, 1024, 0);
...
--*/
{
  ...
  Status = UnicodeStrToAsciiStrS (Buffer, BufferTmp, BufferSize);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  //
...
--*/
{
  ...
  if (gSerialMonitorInterface != NULL) {
    FreePool (gSerialMonitorInterface);
  }
