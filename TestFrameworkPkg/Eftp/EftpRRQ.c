...
--*/
{
  ...
  Rrq = AllocateZeroPool (sizeof (EFTP_RRQ_STATE));
  if (Rrq == NULL) {
  ...
  InitializeListHead (&Rrq->LostPacketList);
...
--*/
{
  ...
  if (!IsListEmpty (&Rrq->LostPacketList)) {
    EftpRrqFlushLostBlock (Private);
  }

  FreePool (Private->RrqState);
...
--*/
{
  ...
  LostBlockEntry = AllocatePool (sizeof (EFTP_LOSTBLOCK_ENTRY));
  ...
  InsertTailList (&Private->RrqState->LostPacketList, &LostBlockEntry->Entry);
...
--*/
{
  ...
  //
  for (ListEntry = (&Private->RrqState->LostPacketList)->ForwardLink; ListEntry != (&Private->RrqState->LostPacketList); ListEntry = ListEntry->ForwardLink) {
    LostBlockEntry = BASE_CR (ListEntry, EFTP_LOSTBLOCK_ENTRY, Entry);
    ...
    if (BlkNo >= LostBlockEntry->Start && BlkNo <= LostBlockEntry->End) {
      ...
      if (LostBlockEntry->Start == LostBlockEntry->End) {
        ...
        //
        Temp = AllocatePool (sizeof (EFTP_LOSTBLOCK_ENTRY));
        if (Temp == NULL) {
          Private->Result = EFI_OUT_OF_RESOURCES;
          return EFI_OUT_OF_RESOURCES;
        }
...
--*/
{
  ...
  while (!IsListEmpty (&Private->RrqState->LostPacketList)) {

    LostBlockEntry = BASE_CR (
                      (&Private->RrqState->LostPacketList)->ForwardLink,
                      EFTP_LOSTBLOCK_ENTRY,
                      Entry
                      );

    RemoveEntryList (&LostBlockEntry->Entry);
    FreePool (LostBlockEntry);
  }
