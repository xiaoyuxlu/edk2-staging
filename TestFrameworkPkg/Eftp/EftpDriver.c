...
--*/
{
  ...
  //
  SbInstance = AllocatePool (sizeof (EFTP_SERVICE_BINDING_PRIVATE));
  if (SbInstance == NULL) {
    EFTP_DEBUG_ERROR ((L"DriverBindingStart: Allocate service binding buffer failed.\n"));
    return EFI_OUT_OF_RESOURCES;
  }

  CopyMem (SbInstance, &gEftpsbDataTemplate, sizeof (EFTP_SERVICE_BINDING_PRIVATE));
  ...
  InitializeListHead (&SbInstance->ChildrenList);

  //
  // BUGBUG: Invalid TPL level
  //
  EfiInitializeLock (&SbInstance->ChildrenListLock, (TPL_CALLBACK + 1));
...
--*/
{
  ...
  //
  Status = EfiAcquireLockOrFail (&ServiceBindingPrivate->ChildrenListLock);
  if (EFI_ERROR (Status)) {
  ...
  //
  IoPrivate = AllocateZeroPool (sizeof (EFTP_IO_PRIVATE));
  if (IoPrivate == NULL) {
    EfiReleaseLock (&ServiceBindingPrivate->ChildrenListLock);
    return EFI_OUT_OF_RESOURCES;
  }
  //
  ...
  //
  CopyMem (&IoPrivate->Eftp, &gEftpProtocolTemplate, sizeof (EFI_EFTP_PROTOCOL));
  ...
  //
  ServiceBindingPrivate->ChildrenNum++;
  InsertTailList (&ServiceBindingPrivate->ChildrenList, &IoPrivate->List);
...
--*/
{
  ...
  //
  if (EFI_ERROR (EfiAcquireLockOrFail (&ServiceBindingPrivate->ChildrenListLock))) {
    IoPrivate->Destroyed = FALSE;
    return EFI_ACCESS_DENIED;
  }

  ASSERT (ServiceBindingPrivate->ChildrenNum >= 1);
  RemoveEntryList (&IoPrivate->List);
  ServiceBindingPrivate->ChildrenNum--;
