...
--*/
{
  ...
  OldTpl = gBS->RaiseTPL (TPL_CALLBACK+1);
...
--*/
{
  ...
  InitializeListHead (&Private->PendingPacketList);
...
--*/
{
  ...
  CopyMem ((VOID *) &ModeData->ConfigData, (VOID *) &Private->ConfigData, sizeof (EFI_EFTP_CONFIG_DATA));
...
--*/
{
  ...
  gBS->SetTimer (
        Private->TimeoutEvent,
        TimerRelative,
        EFI_TIMER_PERIOD_SECONDS(Sec) + EFI_TIMER_PERIOD_MILLISECONDS(MSec)
        );
  return EFI_SUCCESS;
}
