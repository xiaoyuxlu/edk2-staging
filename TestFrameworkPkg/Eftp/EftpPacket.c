...
STATIC LIST_ENTRY mEftpFreePacketList[2];
...
--*/
{
  ...
  Buf = (EFTP_PACKET_BUFFER *) AllocateZeroPool (sizeof (EFTP_PACKET_BUFFER));
  ...
  if (DataSize != 0) {
    Packet = (EFI_EFTP_PACKET *) AllocatePool (DataSize);
    if (Packet == NULL) {
...
--*/
{
  ...
  //
  for (Entry = (Head)->ForwardLink; Entry != (Head); Entry = Entry->ForwardLink) {
    Buf = BASE_CR (Entry, EFTP_PACKET_BUFFER, List);
...
--*/
{
  ...
  CopyMem (NewBuf->Packet, OldBuf->Packet, OldBuf->PacketSize);
...
--*/
{
  ...
  for (Index = 0; Index < 2; Index++) {
    InitializeListHead (&mEftpFreePacketList[Index]);
  }
...
--*/
{
  ...
  for (Index = 0; Index < 2; Index++) {
    ...
    while (!IsListEmpty (Head)) {
      ...
      Buf = BASE_CR (Head->ForwardLink, EFTP_PACKET_BUFFER, List);

      RemoveEntryList (&Buf->List);
      EftpFreePacket (Buf);
    }
  }
...
--*/
{
  ...
  InsertTailList (&Private->PendingPacketList, &Buf->List);
...
--*/
{
  ...
  Len = EFTP_HEADER_LEN + (UINT32) AsciiStrLen (Err->Desc) + 1;
  ...
  Buf->EftpMnpTxData.DestinationAddress = AllocatePool (sizeof (EFI_MAC_ADDRESS));
  Buf->EftpMnpTxData.SourceAddress      = AllocatePool (sizeof (EFI_MAC_ADDRESS));
  CopyMem (
...
--*/
{
  ...
  Len = EFTP_HEADER_LEN + (UINT32) AsciiStrLen (Filename) + (UINT32) AsciiStrLen (Mode) + 2;
  ...
  if ((NOption != 0) && (OptionList != NULL)) {
    ...
    for (Index = 0; Index < NOption; Index++) {
      ...
      Len += (UINT32) AsciiStrLen (OptionList[Index].OptionStr) + (UINT32) AsciiStrLen (OptionList[Index].ValueStr) + 2;
    }
  ...
  P = AsciiStrCpy (P, Filename);
  P = AsciiStrCpy (P, Mode);

  if (NOption != 0 && OptionList != NULL) {
    for (Index = 0; Index < NOption; ++Index) {
      P = AsciiStrCpy (P, OptionList[Index].OptionStr);
      P = AsciiStrCpy (P, OptionList[Index].ValueStr);
    }
  }
...
--*/
{
  ...
  for (Cur = (Head)->ForwardLink; Cur != (Head); Cur = Cur->ForwardLink) {
    if (Cur == Entry) {
      return TRUE;
    }
  }
