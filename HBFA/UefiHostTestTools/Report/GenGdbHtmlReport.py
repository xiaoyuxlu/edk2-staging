...
class GenGdbHtmlReport(object):
    def __init__(self, cfgPath, outputPath, Item):
        ...
        self.__newPathFile = os.path.join(outputPath, "IndexTemplate.html".replace('Template', self.__Item.capitalize() if self.__Item else 'Gdb'.capitalize()))

    def __Str2HyperLink(self, oldStr, link):
        return oldStr.replace(link, '<a href=%s>%s</a>' % (link, link)) if len(link.split('/' if '/' in link else '\\')) > 1 else oldStr
    ...
    def __ParseFailure(self, oriInfo):
        ...
        for index in range(startPos, len(InfoList)):
            ...
            if "#0" not in Info:
                if " at " in Info:
                    ...
                    if len(obj) != 0:
                        lineNum = Info.split(':')[-1] if Info.split(':')[-1] else "--"
                        filePath = obj[0]
                        break
                elif " from " in Info:
                    lineNum = "--"
        ...
        for index in range(len(InfoList)):
            if " at " in InfoList[index]:
                ...
                if len(obj) != 0:
                    InfoList[index] = self.__Str2HyperLink(InfoList[index], obj[0])
            elif " from " in InfoList[index]:
                InfoList[index] = self.__Str2HyperLink(InfoList[index], InfoList[index].split(' from ')[-1])
            elif " in " in InfoList[index]:
                InfoList[index] = self.__Str2HyperLink(InfoList[index], InfoList[index].split(' in ')[-1])
    ...
    def __InsertContent(self):
        ...
        itemDetail = "crashes'" if self.__Item.lower() == 'crashes' else ("hangs'" if self.__Item.lower() == 'hangs' else "failures'")
        itemNum = "Crash" if self.__Item.lower() == 'crashes' else ("Hang" if self.__Item.lower() == 'hangs' else 'Failure')
        testMethod = "AFL" if self.__Item.lower() in ['crashes', 'hangs'] else ("libFuzzer" if self.__Item else "Peach")
        ...
        for section in sections:
            ...
            failType = section
            filePath, lineNum, errorMessage, stackInfo = self.__ParseFailure(info)
            for line in contentTemplate:
                if '$failure' in line:
                    line = line.replace('$failure', os.path.basename(failType).split(',')[0] if self.__Item else ('--' if errorMessage == '--' else os.path.dirname(failType.split(os.path.dirname(os.path.dirname(os.path.dirname(failType))) + ('/' if '/' in failType else '\\'))[-1])))
                if '$seedsPath' in line:
                    line = line.replace('$seedsPath', os.path.dirname(failType))
                if '$seedsNum' in line:
                    line = line.replace('$seedsNum', TotalSeedNum)
                if '$filePath' in line:
                    if len(filePath.split('/' if '/' in filePath else '\\')) > 1:
                        line = line.replace('$filePath', filePath).replace('$fileName', os.path.basename(filePath))
                    else:
                        line = line.replace('<a href="$filePath">$fileName</a>', os.path.basename(filePath))
                if '$lineNum' in line:
