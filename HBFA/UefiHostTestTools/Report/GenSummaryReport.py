...
class GenSummaryReport(object):
    def __init__(self, execfile, outputPath, methods, reportType, failNum, totalNum, execTime):
        self.__testCaseName = os.path.basename(execfile)
        self.__totalNum = totalNum
        self.__failNum = failNum
        self.__execTime = str(execTime).split(':')[0] + ' hrs, ' + str(execTime).split(':')[1] + ' mins, ' + \
                          str(execTime).split(':')[2] + ' secs'
        self.__methods = methods
        self.__reportType = reportType
        self.__oldPath = os.path.dirname(os.path.realpath(__file__))
        self.__oldPathFile = os.path.join(self.__oldPath, "IndexSummaryTemplate.html")
        self.__newSanitizerPathFile = os.path.join(outputPath, "SanitizerSummaryReport.html")
        self.__newGdbPathFile = os.path.join(outputPath, "GdbSummaryReport.html")
    ...
    def __InsertContent(self, insertElem, newreportPath):
        if self.__methods == 'afl':
            testMethod='AFL'
        elif self.__methods == 'peach+sanitizer':
            testMethod='Peach+Sanitizer'
        else:
    ...
    def __GenAFLGdbSumReport(self):
        totalList = self.__AddContentList('Total Case Numbers:', self.__totalNum, '')
        crashesList = self.__AddContentList('Crashes Case Numbers:', self.__failNum[0],
                                            self.__crashreportlink)
        hangsList = self.__AddContentList('Hangs Case Numbers:  ', self.__failNum[1], self.__hangreportlink)
        execTImeList = self.__AddContentList('Execution Time:  ', self.__execTime, '')
    ...
    def __GenOtherSumReport(self):
        ...
        if self.__reportType == 'gdb':
            failList = self.__AddContentList('Failure Type Numbers:  ', self.__failNum,
            ...
            execTImeList = self.__AddContentList('Execution Time:  ', self.__execTime, '')
            contentList = totalList + failList + execTImeList
            self.__InsertContent(''.join(contentList), self.__newSanitizerPathFile)
