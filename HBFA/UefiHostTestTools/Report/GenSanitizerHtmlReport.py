...
class GenSanitizerHtmlReport(object):
    def __init__(self, reportLogPath, item, outputPath):
        ...
        if self.__classification == '':
            self.testMethod = 'Peach'
        elif self.__classification == 'crashes' or self.__classification == 'hangs':
            self.testMethod = 'AFL'
        else:
        ...
        self.__AddressSanitizerLog = os.path.join(self.__reportLogPath, 'AddressSanitizer.cfg')
        self.__LeakSanitizerLog = os.path.join(self.__reportLogPath, 'LeakSanitizer.cfg')

    def __GetContentsInfo(self, contents):
        contents_list = contents.split('\n')
        error_message = '--'
        for content in contents_list:
            match1 = re.search('ERROR: AddressSanitizer: (\S)+', content)
            if match1:
            ...
            if match2:
                ...
                FileName = os.path.basename(FilePath.replace(':\\','\\').split(':')[0])
                TmpLink = FilePath.replace(':\\','\\').split(':')[0].split('\\')
                TmpLink[0] = TmpLink[0].replace(TmpLink[0], TmpLink[0] + ':') if len(TmpLink) > 1 else TmpLink[0]
                FileLink = '\\'.join(TmpLink)
            if 'allocated by' in content or re.search('Address 0x(\w)+ is located in', content):
                break
    ...
    def __InsertContent(self, insertElem, failNum):
        ...
        for i, line in enumerate(fileData):
            if '$Date' in line:
                line = line.replace(line, '<td class="DateValue">%s</td>' % date)
                fileData[i] = line
            ...
            if '$itemNum' in line:
                ...
                fileData[i] = line
            if '$failNum' in line:
                line = line.replace(line, '<td class="Numbervalue">%s</td>' % failNum)
                fileData[i] = line
                break
    ...
    def GenHtml(self):
        ...
        if os.path.exists(self.__AddressSanitizerLog):
            ...
            for item in sections:
                amatch = re.search('number(\d)+_AddressSanitizer', item)
                lmatch = re.search('number(\d)+_LeakSanitizer', item)
                if (not amatch) and (not lmatch):
                    seedsPath = os.path.dirname(item)
                    if self.__classification != '':
                        if os.path.basename(seedsPath) in ['crashes','hangs']:
                            failType = os.path.join(os.path.basename(seedsPath), os.path.basename(item).split(',')[0])
                        else:
                            failType = os.path.basename(item)
                    else:
                        failType=os.path.basename(os.path.dirname(os.path.dirname(item)))+'/'+os.path.basename(os.path.dirname(item))
                else:
                    failType = item
                    seedsPath = os.path.join(self.__reportLogPath, 'FailureSeeds', failType)
                    self.testMethod = 'Peach+Sanitizer'
                section_contents = self.__cfg.get(item, 'stackinfo')
                totalseednum = self.__cfg.get(item, 'totalseedsnum')
                afailNum = len(sections)
                if section_contents != '':
                    fileName, fileLink, lineNum, errorMessage = self.__GetContentsInfo(section_contents)
                    ...
                    for sline in stack_info:
                        ...
                        if match:
                            ...
                            FileLink = '\\'.join(TmpLink)
                            if '(' in FileLink:
                                FileLink = FileLink.lstrip('\(')
                                FileLink = FileLink.rstrip('\)')
                            sline = sline.replace(sline, a + ('<a href=%s>' % FileLink) + ' ' + FilePath + '</a>')
                        sline = sline + '<br>'
                ...
                for line in contents:
                    ...
                    if '$filePath' in line:
                        if fileLink == '':
                            line = line.replace('<a href="$filePath">', '').replace('</a>', '')
                        else:
                    ...
                    if '$seedsPath' in line:
                        if seedsPath == '':
                            line = line.replace('<a href="$seedsPath">', '').replace('</a>', '')
                        else:
...
#                 lfailType = item
#                 if self.__lcfg.has_option(item, 'filepath'):
#                     lseedsPath = self.__lcfg.get(item, 'filepath')
#                 else:
#                     lseedsPath = os.path.join(self.__reportLogPath, 'FailureSeeds', lfailType)
                amatch1 = re.search('number(\d)+_AddressSanitizer', item)
                lmatch1 = re.search('number(\d)+_LeakSanitizer', item)
                if (not amatch1) and (not lmatch1):
                    lseedsPath = os.path.dirname(item)
                    if self.__classification != '':
                        if os.path.basename(lseedsPath) in ['crashes','hangs']:
                            lfailType = os.path.join(os.path.basename(lseedsPath), os.path.basename(item).split(',')[0])
                        else:
                            lfailType = os.path.basename(item)
                    else:
                        lfailType=os.path.basename(os.path.dirname(os.path.dirname(item)))+'/'+os.path.basename(os.path.dirname(item))
                else:
                    lfailType = item
                    lseedsPath = os.path.join(self.__reportLogPath, 'FailureSeeds', lfailType)
                    self.testMethod = 'Peach+Sanitizer'
                ...
                lfileName, lfileLink, llineNum, lerrorMessage = self.__GetLContentsInfo(section_contents)
                stack_info = section_contents.split('\n')
                ...
                for sline in stack_info:
                    ...
                    if match:
                        FilePath = match.group().split(' ')[-1]
                        a = ' '.join(match.group().split(' ')[:-1])
                        LtmpLink = FilePath.replace(':\\','\\').split(':')[0].split('\\')
                        LtmpLink[0] = LtmpLink[0].replace(LtmpLink[0], LtmpLink[0] + ':') if len(LtmpLink) > 1 else LtmpLink[0]
                        lfileLink = '\\'.join(LtmpLink)
                        if '(' in lfileLink:
                            lfileLink = lfileLink.lstrip('\(')
                            lfileLink = lfileLink.rstrip('\)')
                        sline = sline.replace(sline, a + ('<a href=%s>' % lfileLink) + ' ' + FilePath + '</a>')
                    sline = sline + '<br>'
