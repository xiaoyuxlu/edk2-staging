...
class errorInjection():
    ...
    def run(self):
        ...
        for CountNum in range(1, self.CallErrorCountNum + 1):
            ...
            for count in range(3):
                ...
                self.create_tcs(os.path.join(Case_Path, CaseName), CountNum, count)
    ...
    def create_tcs(self, tcs_file, num, count):
        ...
        if count == 0:
            ...
            conf.add_section('ReadBlocks')
            conf.set('ReadBlocks', 'CallErrorCount', num)
            conf.set('ReadBlocks', 'ReturnValue', 'EFI_DEVICE_ERROR')
        elif count == 2:
            conf.add_section('ReadDisk')
            conf.set('ReadDisk', 'CallErrorCount', num)
            conf.set('ReadDisk', 'ReturnValue', 'EFI_DEVICE_ERROR')
        else:
            pass
...
if __name__ == '__main__':
    # # # Opt Parser
    parse = OptionParser()
    parse.add_option("-c", dest="CallErrorCountNum", metavar=" ", help="CallErrorCount number,if CallErrorCount = N,\
 script will try CallErrorCount = 1 ~ CallErrorCount = N", default=None)
